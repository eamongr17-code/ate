...
| Design system | `App/DesignSystem/` — palette/type/shadow tokens, ported line icons, slip/row/pill/sheet components; `design/v1/*.dc.html` is the pixel reference, built to parity | The ratified look lives in code once and everywhere. *Rejected: generated-token machinery; Figma as source of truth (the canvas snapshot is).* |
...
| CI | GitHub Actions for everything: PRs (lint/build/AteKit tests/sorter tests/contract, <10 min) and Beta archive → TestFlight internal on every merge to main, signed with an App Store Connect API key plus ONE held development certificate in secrets (Apple's cert cap made cloud-minted certs untenable) | Signing is still Apple's problem, but one CI system beats two. *Rejected: fastlane + match; Xcode Cloud.* |
