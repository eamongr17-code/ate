...
| **Log "Where" search-blend** (`pick.tsx`) | Places autocomplete ∪ fuzzy `source='manual'` restaurants, merged server-side in `places-search` op=autocomplete (`results[]`, capped 5, discriminated). Manual half = `search_manual_restaurants(q)` RPC (0015). | — (read) | **partial GIN trigram** `restaurants_manual_name_trgm WHERE source='manual'` (0015); word_similarity (`<%`) + ILIKE, threshold 0.45. See `manual-search-blend-contract.md`. |
...
- `RPC search_manual_restaurants(p_query, p_limit?)` → fuzzy (`pg_trgm`) name search over `source='manual'` restaurants; returns `id/name/city/cuisine/match_score/strong`, strongest-first (MANUAL-RESTO search-blend, 0015). Called by the `places-search` edge fn (op=autocomplete) and merged into the discriminated `results[]` blend (Places predictions + manual matches, capped 5). `authenticated`+`service_role` EXECUTE; SECURITY INVOKER, read-only. Contract: `manual-search-blend-contract.md`.
