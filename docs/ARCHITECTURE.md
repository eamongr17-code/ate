...
| CI | GitHub Actions on PRs (lint/build/test/contract, <10 min); Xcode Cloud for signing → TestFlight internal on every merge to main | Signing is Apple's problem, not a fastlane config. *Rejected: fastlane + match.* |
