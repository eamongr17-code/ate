...
struct DetailDeletionTests {
    ...
    private func card(_ id: UUID, isMine: Bool = false) -> EntryCard {
        EntryCard(
            id: id, authorID: author.id, body: "Good.", orderNumber: 1, sortStatus: .sorted,
            createdAt: Date(timeIntervalSince1970: 1_789_776_000), isMine: isMine, author: author,
    ...
    @Test("Place → your visit → Delete → back: the visit is gone from the place page, both lists")
    func placeDropsTheDeletedVisit() async {
        let source = FakePlaceDishSource()
        let mine = UUID()
        let theirs = UUID()
        source.seed(
            place: PlaceSummary(restaurantID: placeID, name: "Tipo 00", myVisits: 1),
            entries: [card(mine, isMine: true), card(theirs), card(UUID())]
        )
        let deletions = EntryDeletions()
        let place = PlacePageStore(restaurantID: placeID, places: source, deletions: deletions)
        await place.load()
        #expect(place.visits.entries.map(\.id) == [mine])
        #expect(place.entries.entries.count == 2)

        deletions.send(mine)
        #expect(place.visits.entries.isEmpty, "your deleted visit is still on the place page")
        deletions.send(theirs)
        #expect(place.entries.entries.map(\.id).contains(theirs) == false)
        #expect(place.entries.entries.count == 1)
    }

    @Test("A deleted visit's review leaves the dish page in the same turn")
    func dishDropsTheDeletedReview() async {
        let source = FakePlaceDishSource()
        let gone = UUID()
        source.seed(
        ...
        deletions.send(dish.reviews[0].entryID ?? UUID())
        #expect(dish.phase == .empty)
    }
}
