...
struct EntrySlip: View {
    ...
    private func paper(@ViewBuilder _ content: () -> some View) -> some View {
        content()
            ...
            // on its first 44pt dish row, which carries its own air. The feed's are `10px 14px 12px`.
            .padding(.top, slip.byline == nil ? AteMetrics.slipPaddingTopBare : insets.top)
            .padding(.horizontal, insets.side)
            .padding(.bottom, insets.bottom + AteMetrics.tornEdgeHeight)
            .frame(maxWidth: .infinity, alignment: .leading)
            .ateSlip()
            .ateTornPaper()
    }

    /// The paper's padding and the gap between its bands, by density.
    private var insets: SlipInsets { SlipInsets(slip.density) }
        ...
        VStack(alignment: .leading, spacing: 0) {
            ForEach(Array(slip.dishes.enumerated()), id: \.element.id) { index, dish in
                VStack(spacing: 0) {
                    ...
                    SlipDishRow(
                        dish: dish,
                        action: dishAction(dish, interactive: interactive),
...
/// A slip's padding and band gap: `12/16/14` and `10`, or the feed's `10/14/12` and `8` (`FeedTight`).
private struct SlipInsets {
    let top: CGFloat
    let side: CGFloat
    let bottom: CGFloat
    let gap: CGFloat

    init(_ density: AteSlip.Density) {
        switch density {
        case .regular:
            top = AteMetrics.slipPaddingTop
            side = AteMetrics.slipPadding
            bottom = AteMetrics.slipPaddingBottom
            gap = AteMetrics.slipBandGap
        case .tight:
            top = AteMetrics.slipTightPaddingTop
            side = AteMetrics.slipTightPadding
            bottom = AteMetrics.slipTightPaddingBottom
            gap = AteMetrics.slipTightBandGap
        }
    }
}
