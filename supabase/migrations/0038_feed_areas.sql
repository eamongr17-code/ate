...
--
--   feed_areas()           → table(area text, entry_count int): every area with entries the Feed would
...
-- `p_area` gets today's feed. New RPC `feed_areas()` for authenticated and anon.
...
-- ===========================================================================
-- 2. feed_areas — same dispatch shape as every browse read.
-- ===========================================================================
...
as $$
  ...
  select mode() within group (order by a.area), count(*)::int
  from (
  ...
  order by 2 desc, 1;
$$;
revoke all on function browse.feed_areas() from public;
grant execute on function browse.feed_areas() to anon;
...
begin
  if current_user = 'anon' then
    return query select * from browse.feed_areas();
    return;
  end if;
...
comment on function public.feed_areas() is
  'The Feed area picker (0038): place_locality() of every entry the Feed would show the viewer (own entries excluded; blocked/deactivated authors excluded), with its entry count, busiest first then A-Z. Pass a row''s `area` to get_entry_feed(p_area). anon → browse twin (all public entries).';

revoke all on function public.feed_areas() from public;
grant execute on function public.feed_areas() to authenticated, anon, service_role;
